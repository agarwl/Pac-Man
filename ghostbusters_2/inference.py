...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = self.legalPositions * (self.numParticles//numPositions)
        remaining = self.numParticles % numPositions
        self.particles += self.legalPositions[0:remaining]
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                # update the belief for a position using prior belief and 
                # likelihood of obtaining noisyDistance  given that position
                new_beliefs[p] = beliefs[p] * emissionModel[trueDistance]

        if(new_beliefs.totalCount() == 0):
            self.initializeUniformly(gameState)
        else:
            new_beliefs.normalize()
            self.particles = [util.sample(new_beliefs) for _ in xrange(self.numParticles)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        # "*** YOUR CODE HERE ***"
        legalPositionTuples = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(legalPositionTuples)
        numPositions = len(legalPositionTuples)
        remaining = self.numParticles % numPositions
        self.particles = legalPositionTuples * (self.numParticles//numPositions)
        self.particles += legalPositionTuples[0:remaining]
